...
| `skill-script-compound-guard.sh` | `;`/`&&`/`\|\|`로 연결된 복합 명령 안의 worktree.sh 호출 | `approve-worktree-skill.sh`는 정확한 단일 명령 형태만 매칭하고 셸 연산자가 하나라도 있으면 위임(defer)하므로, worktree.sh를 다른 명령과 체이닝하면 힌트 없이 프롬프트가 뜨고 무인 `/loop`이 멈춥니다. 이 가드는 그 형태를 거부하고 "각 작업을 별도 Bash 호출로 실행하라"는 교정 힌트를 내보내 재시도가 자동 승인되게 합니다. `approve-worktree-skill.sh`의 짝(sibling) 가드이니 같은 opt-in 블록에 연결하세요(`./install.sh --worktree --guards`). 단독 `cd <abs> &&` 접두사와 인용된 구분자는 위임하고, 순수 pipe(`\| tail`)는 범위 밖입니다. |
