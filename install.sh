...
#   ./install.sh --all        # also install the bash-guards, repo-radar + worktree + temp-file skills
...
#   ./install.sh --temp-file  # core hooks + temp-file skill
...
WANT_GUARDS=0; WANT_RADAR=0; WANT_WORKTREE=0; WANT_TEMPFILE=0
for a in "$@"; do
  case "$a" in
    --all)       WANT_GUARDS=1; WANT_RADAR=1; WANT_WORKTREE=1; WANT_TEMPFILE=1 ;;
    --guards)    WANT_GUARDS=1 ;;
    --radar)     WANT_RADAR=1 ;;
    --worktree)  WANT_WORKTREE=1 ;;
    --temp-file) WANT_TEMPFILE=1 ;;
...
# --- temp-file skill (opt-in) — SKILL.md only; documents the ~/tmp convention temp-dir-guard.sh enforces ---
if [ "$WANT_TEMPFILE" = 1 ]; then
  copy "$SRC/skills/temp-file/SKILL.md" "$DEST/skills/temp-file/SKILL.md"
fi
